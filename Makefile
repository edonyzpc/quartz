...
serve: ## Serve Quartz locally
	...
	hugo server --disableFastRender --enableGitInfo --minify --bind=$(or $(HUGO_BIND),0.0.0.0) --baseURL=$(or $(HUGO_BASEURL),http://localhost) --port=$(or $(HUGO_PORT),1313) --appendPort=$(or $(HUGO_APPENDPORT),true) --liveReloadPort=$(or $(HUGO_LIVERELOADPORT),-1)

docker: ## Serve locally using Docker
	docker run -it --volume=$(shell pwd):/quartz -p 1313:1313 ghcr.io/jackyzha0/quartz:hugo

cleanup: ## cleanup for rebuilding
	rm -rf resources/ assets/indices/linkIndex.json assets/indices/contentIndex.json